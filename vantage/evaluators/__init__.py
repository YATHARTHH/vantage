...
class LocalHeuristicEvaluator:
    ...
    Returns a :class:`RAGQualityResult` with all signals independently set.
    """
    ...
    def evaluate(
        ...
        """Run evaluation. Returns skipped result if inputs are missing."""
        ...
        # A response sentence is "supported" if it shares >30% token overlap with any context sentence.
        supported = 0
        ...
        # Fraction of factual claims (numbers / absolutes) in response absent from context
        factual_claims = _FACTUAL_CLAIM_RE.findall(response_text)
        unsupported_claims = 0
        for claim in factual_claims:
            if claim.lower() not in context_combined.lower():
        ...
        # Token-level overlap: |response ∩ context| / |response|
        overlap_ratio = (
        ...
        # Proportion of context sentences referenced in the response
        utilized = 0
        for c_sent in context_sentences:
            c_tokens = _tokenize(c_sent)
            if not c_tokens:
                continue
            if len(c_tokens & response_tokens) / len(c_tokens) >= 0.20:
                utilized += 1
        utilization = utilized / len(context_sentences) if context_sentences else 0.0

        # 5. safety_heuristic (independent signal)
